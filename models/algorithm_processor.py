...
class LitAlgorithmProcessor(pl.LightningModule):
    ...
    def __init__(
        ...
        for algo in algorithm_names:
            self.algorithms[algo] = algo_classes[algo](
                ...
                dataset_class=_DATASET_SPECS[algo]["dataclass"],
                dataset_root=_DATASET_SPECS[algo]["rootdir"],
    ...
    def train_dataloader(self):
        return [
            self.algorithms[algo].train_dataloader() for algo in self.algorithm_names
        ]

    def val_dataloader_normal(self):
        return CombinedLoader(
            dict(
                (name, algo.val_dataloader()) for name, algo in self.algorithms.items()
            ),
            mode="max_size_cycle",
        )
    ...
    def test_dataloader(self, suffix=""):
        return CombinedLoader(
            dict(
                (name, algo.test_dataloader(suffix=suffix))
    ...
    def fwd_step(self, batch, batch_idx):
        assert not self.freeze_proc or not any(
            k.requires_grad for k in self.processor_set.processors[0].parameters()
        ), breakpoint()
        outputs = {}
    ...
    def training_step(self, batch, batch_idx):
        ...
        for name, algo_batch in zip(self.algorithm_names, batch):
            ...
            algorithm.global_step = self.global_step
            ...
            self.log_dict(
                dict(
                    (f"train/loss/{name}/{k}", v)
                    for k, v in output["losses_dict"].items()
                ),
                batch_size=num_graphs,
            )
            self.log(
                f"train/loss/{name}/average_loss",
                output["loss"],
                on_step=True,
                on_epoch=True,
                batch_size=num_graphs,
            ...
            self.log_dict(
                dict(
                    (f"train/acc/{name}/{k}", v)
                    for k, v in output["accuracies"].items()
                ),
                batch_size=num_graphs,
    ...
    def valtest_step(self, batch, batch_idx, mode):
        ...
        for name, algorithm in self.algorithms.items():
            ...
            self.log_dict(
                dict(
                    (f"{mode}/loss/{name}/{k}", v)
                    for k, v in output[name]["losses"].items()
                ),
                batch_size=batch[name].num_graphs,
                add_dataloader_idx=False,
            )
            average_loss = sum(output[name]["losses"].values()) / len(
                output[name]["losses"]
            )
            self.log(
                f"{mode}/loss/{name}/average_loss",
                average_loss,
                batch_size=batch[name].num_graphs,
                add_dataloader_idx=False,
                on_epoch=True,
            ...
            self.log_dict(
                dict(
                    (f"{mode}/acc/{name}/{k}", v)
                    for k, v in output[name]["accuracies"].items()
                ),
                batch_size=batch[name].num_graphs,
                add_dataloader_idx=False,
            )
    ...
    def on_validation_epoch_end(self):
        self.min_val = min(
            self.min_val, self.trainer.logged_metrics.get("val/loss/average_loss", 100)
        )
        self.log(f"val/loss/min_loss_so_far", self.min_val, add_dataloader_idx=False)
    ...
    def configure_optimizers(self):
        ...
        optimizer = optim.Adam(
            [{"params": term_params, "lr": lr}, {"params": normal_params, "lr": lr}],
            lr=lr,
